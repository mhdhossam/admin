...
const tokenStorage = {
  ...
  clear: () => { localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(REFRESH_KEY); localStorage.removeItem("admin_user"); },
};
...
async function apiFetch(path, options = {}, _retry = true) {
  ...
  if (res.status === 401 && _retry) {
    try { const t = await refreshAccessToken(); headers.set("Authorization", `Bearer ${t}`); return apiFetch(path, options, false); }
    catch { throw new ApiError(401, { detail: "Session expired." }); }
  }
  if (!res.ok) {
    ...
    <div style={S.loginWrap}>
      <div style={S.loginCard}>
        ...
        <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <input style={S.input} type="text" autoFocus placeholder="admin"
          ...
          <button style={{...S.btn, ...S.btnPrimary, marginTop:8}} type="submit" disabled={loading}>
            {loading ? <span style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center"}}>{Icon.spinner} Authenticating…</span> : "Sign In"}
          </button>
        </form>
      </div>
    </div>
  ...
  return (
    <aside style={S.sidebar}>
      ...
      <div style={S.sidebarSection}>
        ...
        <button style={{...S.navItem, ...(location.pathname==="/admin" ? S.navItemActive : {})}}
...
function DynamicTable({ schema, data, onEdit, onDelete, loading }) {
  ...
  function renderCell(field, value) {
    ...
    if (field.type === "boolean") return value ? <span style={S.badge.green}>{Icon.check} Yes</span> : <span style={S.badge.red}>{Icon.x} No</span>;
  ...
  if (loading) return (
    <div style={S.tableLoadingWrap}>
      {Icon.spinner}
      <span style={{marginLeft:12, opacity:.6}}>Loading records…</span>
    </div>
  );
    ...
    <div style={S.tableWrap}>
      <table style={S.table}>
        <thead>
          <tr>
            {listFields.map(f => (
              <th key={f.name} style={S.th}>{f.label}</th>
            ))}
        ...
        <tbody>
          ...
          {data?.map((row, i) => (
            <tr key={row.id ?? i} style={S.tr}>
              {listFields.map(f => (
                <td key={f.name} style={S.td}>{renderCell(f, row[f.name])}</td>
              ))}
...
function DynamicForm({ schema, initial, onSubmit, onCancel, loading }) {
  ...
  const [values, setValues] = useState(() => {
    const init = {};
    editableFields.forEach(f => { init[f.name] = initial?.[f.name] ?? ""; });
    return init;
  });
  const [relationOptions, setRelationOptions] = useState({});
  const [errors, setErrors] = useState({});

  // Load relation dropdown options
  useEffect(() => {
  ...
  function renderField(f) {
    ...
    if (f.type === "text") return (
      <textarea style={{...style, minHeight:100, resize:"vertical"}}
        value={val} onChange={e => set(f.name, e.target.value)} />
    );
    ...
    <form onSubmit={submit} style={S.formGrid}>
      ...
      <div style={{gridColumn:"1/-1", display:"flex", gap:10, justifyContent:"flex-end", marginTop:8}}>
        ...
        <button type="submit" style={{...S.btn, ...S.btnPrimary}} disabled={loading}>
          {loading ? <span style={{display:"flex",alignItems:"center",gap:8}}>{Icon.spinner} Saving…</span> : (initial ? "Update Record" : "Create Record")}
        </button>
      </div>
    </form>
  );
...
function DeleteConfirm({ record, schema, onConfirm, onCancel, loading }) {
  ...
  return (
    <div style={{textAlign:"center", padding:"8px 0"}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"center"}}>
        ...
        <button style={{...S.btn, background:"#ef4444", color:"#fff", border:"none"}} onClick={onConfirm} disabled={loading}>
...
function DynamicPage({ modelName }) {
  ...
  const handleUpdate = async (data) => {
    setSaving(true);
    try {
      await crudApi.update(schema.endpoint, activeRecord.id, data);
      showToast("Record updated");
      setModal(null); setActiveRecord(null); loadList();
    } catch (e) { showToast(e.message, "error"); }
    finally { setSaving(false); }
  };
  ...
  const handleDelete = async () => {
    setSaving(true);
    try {
      await crudApi.delete(schema.endpoint, activeRecord.id);
  ...
  if (schemaLoading) return (
    <div style={S.centerFlex}>{Icon.spinner}<span style={{marginLeft:12,opacity:.6}}>Loading schema…</span></div>
  );
  if (schemaError) return (
    <div style={S.centerFlex}><span style={{color:"#f87171"}}>Schema error: {schemaError}</span></div>
  );
    ...
    <div style={S.pageWrap}>
      ...
      {modal === "create" && (
        <Modal title={`Create ${schema.name}`} onClose={() => setModal(null)}>
          <DynamicForm schema={schema} initial={null}
      ...
      {modal === "edit" && activeRecord && (
        <Modal title={`Edit ${schema.name} #${activeRecord.id}`} onClose={() => { setModal(null); setActiveRecord(null); }}>
          <DynamicForm schema={schema} initial={activeRecord}
            onSubmit={handleUpdate} onCancel={() => { setModal(null); setActiveRecord(null); }} loading={saving} />
        </Modal>
      )}
      ...
      {modal === "delete" && activeRecord && (
        <Modal title="Confirm Deletion" onClose={() => { setModal(null); setActiveRecord(null); }}>
          <DeleteConfirm record={activeRecord} schema={schema}
            onConfirm={handleDelete} onCancel={() => { setModal(null); setActiveRecord(null); }} loading={saving} />
        </Modal>
      )}
    </div>
  );
...
function AdminShell() {
  ...
  // Parse /admin/:model from path
  const match = path.match(/^\/admin\/([^/]+)/);
  const currentModel = match ? match[1] : null;

  return (
    <div style={S.shell}>
      <Sidebar schemas={schemas} currentModel={currentModel} />
...
function App() {
  ...
  if (loading) return (
    <div style={{...S.centerFlex, height:"100vh", background:"#0a0c0f"}}>
      {Icon.spinner}
    </div>
  );
...
const S = {
  ...
  dashCardIcon: { color:"#22d3ee", opacity:.6, marginBottom:4 },
